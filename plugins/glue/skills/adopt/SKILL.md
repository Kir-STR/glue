...
- `materialized[]` — файлы, остающиеся без изменений, но под отслеживанием: `targetPath`, `sourceTemplate` (имя шаблона, если содержимое дословно равно шаблону бандла — у такого модуля оставляй decision `added-from-template`, это сохраняет drift-сигнал; иначе `null`), `kind`, `expectedCurrentHash` (sha256-hex из P2-обзора, обязателен — файл должен существовать). Сюда входят: неизменённый инструкц-файл каждого заявленного движка, нетронутые файлы прошлой доставки, существующие файлы `adopted-existing` модулей;
