...
5. **Обработать результат.**
   - `ok: true` → доставка выполнена. Покажи `manifest` (доставленные движки, файлы). Если в проекте уже есть код или документация — предложи оператору продолжить `/glue:adopt` сразу: он заполнит TODO-заготовки правил из кода и зафиксирует решения в манифесте; ручная правка поставленных файлов мимо adopt будет светиться в `status` как `changed`; канонизирует её только adopt. Не запускай adopt без согласия оператора.
