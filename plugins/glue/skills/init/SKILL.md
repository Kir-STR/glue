...
5. **Обработать результат.**
   - `ok: true` → доставка выполнена. Покажи `manifest` (доставленные движки, файлы). Если в проекте уже есть код или документация — предложи оператору набрать `/glue:adopt` следующей командой (сам не вызывай: скилл закрыт от программного вызова, `disable-model-invocation` — запустить его может только оператор): adopt заполнит TODO-заготовки правил из кода и зафиксирует решения в манифесте; ручная правка поставленных файлов мимо adopt будет светиться в `status` как `changed`; канонизирует её только adopt.
