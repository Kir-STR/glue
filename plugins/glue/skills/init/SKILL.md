...
5. **Обработать результат.**
   ...
   - `conflicts.length > 0` (`ok: false`, exit 0) → покажи каждый конфликт (`targetPath`, `reason` — напр. файл правлён вручную). Перезаписи нет: предложи оператору разрешить конфликт вручную (сохранить или убрать правки), затем повтори `init`. Флаг `--force` удалён — CLI ответит ошибкой.
