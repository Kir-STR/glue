...
test('absent на диске → write с expectedCurrentHash null', () => {
  const r = decidePlan({ targets: [T('.claude/rules/a.md', 'H')], prevManifest: null, diskHashFn: disk({}) })
...
test('current == plannedHash → materialized (recovery)', () => {
  const r = decidePlan({ targets: [T('.claude/rules/a.md', 'H')], prevManifest: null, diskHashFn: disk({ '.claude/rules/a.md': 'H' }) })
...
test('managed и current == writtenHash → write (update)', () => {
  ...
  const r = decidePlan({ targets: [T('.claude/rules/a.md', 'NEW')], prevManifest: prev, diskHashFn: disk({ '.claude/rules/a.md': 'OLD' }) })
  assert.equal(r.writes.length, 1)
...
test('current != plannedHash и unmanaged → conflict', () => {
  const r = decidePlan({ targets: [T('.claude/rules/a.md', 'H')], prevManifest: null, diskHashFn: disk({ '.claude/rules/a.md': 'HAND' }) })
  assert.equal(r.writes.length, 0)
...
test('снятый managed-файл без правок → delete', () => {
  ...
  const r = decidePlan({ targets: [], prevManifest: prev, diskHashFn: disk({ '.claude/rules/old.md': 'W' }) })
  assert.deepEqual(r.deletes, [{ targetPath: '.claude/rules/old.md', expectedCurrentHash: 'W' }])
})

test('снятый правленный файл → conflict', () => {
  const prev = { files: [{ targetPath: '.claude/rules/old.md', writtenHash: 'W' }] }
  const r = decidePlan({ targets: [], prevManifest: prev, diskHashFn: disk({ '.claude/rules/old.md': 'HAND' }) })
...
test('снятый уже отсутствующий файл → ничего', () => {
  const prev = { files: [{ targetPath: '.claude/rules/old.md', writtenHash: 'W' }] }
  const r = decidePlan({ targets: [], prevManifest: prev, diskHashFn: disk({}) })
  assert.equal(r.deletes.length, 0)
  assert.equal(r.conflicts.length, 0)
})
